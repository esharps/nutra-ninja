...
public class PlayerViewerManager : MonoBehaviour {


	private KinectSensor mySensor;
	private MultiSourceFrameReader msFrameReader;
	//public GestureTracker myGestureTracker;
	...
	void Update()
	{
							...
							for (int x = 0; x < depthWidth; ++x)
							{
								...
								if (player != 0xff)
								{
										...
										(colorY < colorHeight))
									{
										...
										displayPixels[displayIndex + 0] = 0x4f; // BLUE colorFrameData[colorIndex]
										displayPixels[displayIndex + 1] = 0x4c; // GREEN colorFrameData[colorIndex + 1];
										displayPixels[displayIndex + 2] = 0x48; // RED colorFrameData[colorIndex + 2];
		...
		/*if (myGestureTracker.RH_GESTURE == 1)
        {
            //Render Line
            Debug.Log("RH GESTURE DETECTED");
        }
        else
        {
            //Don't render
